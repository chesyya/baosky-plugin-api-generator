...
MetaDescription: UX guidelines for the Side Bar in a Baosky 插件.
---

# Sidebars

The Primary and Secondary Sidebars consists of one or more [Views](/api/ux-guidelines/views) contributed by a [View Container](/api/ux-guidelines/views#view-containers). 插件 can contribute Views to an existing View Container (for example, Explorer) or they can contribute an entirely new View Container.
...
- Use an excessive number of View Containers. A single View Container (such as a Sidebar with Views unique to that 插件) is generally enough for most 插件.
...
Many 插件 choose to contribute Views and/or View Containers to the Primary Sidebar given the high visibility it gives content. Use good judgement when adding content here—too much contributed UI can lead to a cluttered experience that can confuse your users.

<!-- 图片已移除 -->

## Secondary Sidebar

As the name implies, the Secondary Sidebar is normally considered a auxiliary location for Views. While 插件 cannot contribute Views directly to it by default, users can drag Views from the Primary Sidebar or the Panel to customize their layout.
...
- [Tree View 插件 sample](https://github.com/microsoft/vscode-插件-samples/tree/main/tree-view-sample)
- [Webview View 插件 sample](https://github.com/microsoft/vscode-插件-samples/tree/main/webview-view-sample)
- [Welcome View 插件 sample](https://github.com/microsoft/vscode-插件-samples/tree/main/welcome-view-content-sample)
