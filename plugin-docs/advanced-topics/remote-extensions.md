...
Native modules bundled with (or dynamically acquired for) a Baosky 插件 must be recompiled [using Electron's `electron-rebuild`](https://electronjs.org/docs/tutorial/using-native-node-modules). However, Baosky Server runs a standard (non-Electron) version of Node.js, which can cause binaries to fail when used remotely.
...
- Contribute to [our documentation](https://github.com/microsoft/vscode-docs) or [Baosky](https://github.com/microsoft/vscode).
