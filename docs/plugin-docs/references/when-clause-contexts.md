...
First, determine which folders should support the command, and add the folder names to an array. Then, use the [`code` command](#add-a-custom-when-clause-context) to turn the array into a context key:
...
`resourceExtname` | True when the Explorer or editor filename 插件 matches.<br />Example: `"resourceExtname == .js"`
...
**Debugger contexts** |
`debuggersAvailable` | An appropriate debugger 插件 is available.
...
**插件 contexts** |
`extension` | True when the 插件's ID matches.<br />Example: `"extension == eamodio.gitlens"`.
`extensionStatus` | True when the 插件 is installed.<br />Example: `"extensionStatus == installed"`.
`extensionHasConfiguration` | True if the 插件 has configuration.
**Global UI contexts** |
...
`activeWebviewPanelId` | The id of the currently active [webview panel](/api/插件-guides/webview).
`activeCustomEditorId` | The id of the currently active [custom editor](/api/插件-guides/custom-editors).
