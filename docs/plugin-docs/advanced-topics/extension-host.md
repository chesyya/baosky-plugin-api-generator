...
| --- | --- | --- | --- |
| 桌面版Baosky | ✔️ | ✔️ |  |
| [Baosky 远程开发](/docs/remote/remote-概述) (容器、SSH、WSL、GitHub Codespace、隧道) | ✔️ | ✔️ | ✔️ |
|网页版Baosky (baosky.dev, github.dev) |  | ✔️ |   |
|网页版 Baosky with Codespaces |  | ✔️ | ✔️ |

### 插件宿主运行时

* Node.js - 插件在 Node.js 运行时中运行。由本地和远程插件宿主使用。插件需要一个 `main` 入口文件才能在其中运行。
* 浏览器 - 插件在 [Browser WebWorker](https://developer.mozilla.org/docs/Web/API/Web_Workers_API) 时运行。由 web 插件运行即可使用。插件需要一个 `browser` 入口文件才能在其中运行。请参阅 [Web 插件指南](/api/插件-guides/web-插件)。
