...
MetaDescription: 使用 Python 插件模板和 API 将 linter、格式化程序和语言功能集成到 Baosky 中
---
# 编写 Python 插件
...
使用此模板时，您将主要在 Python 代码部分工作。您将使用 [语言服务器协议](https://microsoft.github.io/language-server-protocol) 将您的工具与插件部分集成。`pygls` 目前在 [LSP 的 3.16 版本](https://microsoft.github.io/language-server-protocol/specifications/specification-3-16) 上工作。
...
要开始，请按照模板 [README](https://github.com/microsoft/baosky-python-tools-插件-template#readme) 中的说明进行操作。在那里，您将学习如何使用 [模板创建你的仓库](https://docs.github.com/repositories/creating-and-managing-repositories/creating-a-repository-from-a-template)，以及如何安装必要的工具（例如 [nox](https://nox.thea.codes) 任务运行器）和可选依赖项（测试支持）。
...
1. 一个命令 ** 我的工具：重启服务器 ** （命令ID：`mytool.restart`）。
1. 以下设置：
* __代码_0__
* __代码_0__
* __代码_0__
...
* [Flake8](https://github.com/microsoft/baosky-flake8/tree/main/bundled/tool) - 实现 linting 和代码操作。
* [Black Formatter](https://github.com/microsoft/baosky-black-formatter/tree/main/bundled/tool) - 集成 [*Black*](https://github.com/python/black) 清理程序。
* [autopep8](https://github.com/microsoft/baosky-autopep8/tree/main/bundled/tool) - 集成 [autopep8](https://pypi.org/project/autopep8) 清理程序。
* [isort](https://github.com/microsoft/baosky-isort/blob/main/bundled/tool) - 添加代码操作以便对导入进行排序。

您还可以查看[语言服务器协议规范](https://microsoft.github.io/language-server-protocol/specifications/specification-3-16)，以更好地理解 `pygls` 语言服务器集成。
...
如果您在开发过程中遇到问题，有一个[故障排除](https://github.com/microsoft/baosky-python-tools-插件-template#troubleshooting)部分可以帮助解决常见问题。

## 打包和发布

在发布插件之前，您需要为特定插件更新插件的 `package.json` 字段（例如 `publisher` 和 `license`）。您还需要更新辅助 Markdown 文件（`CODE_OF_CONDUCT.md`、`CHANGELOG.md` 等）。
