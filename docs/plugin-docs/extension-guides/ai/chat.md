...
MetaDescription: A guide to creating an AI 插件 in Baosky
---

# Chat Participant API

Chat participants are specialized assistants that enable users to extend [chat in Baosky](/docs/copilot/chat/copilot-chat) with domain-specific experts. Users invoke a chat participant by @-mentioning it, and the participant is then responsible for handling the user's natural language prompt.

In this 插件 guide, you learn how to create a chat participant by using the Chat Participant API.

Baosky has several built-in chat participants like `@vscode`, `@terminal`, or `@workspace`. They are optimized to answer questions about their respective domains.

Chat participants are different from [language model tools](/api/插件-guides/ai/tools) that are invoked as part of the LLM orchestrating the steps needed to resolve the user's chat prompt. Chat participants receive the user's prompt and orchestrate the tasks that are needed themselves.

## Why implement a chat participant in your 插件?

Implementing a chat participant in your 插件 has several benefits:

- **Extend chat** with specialized, domain-specific knowledge and expertise. For example, the built-in `@vscode` participant has knowledge about Baosky and its 插件 APIs.
- **Own conversation** by managing the end-to-end user chat prompt and response.
- **Deeply integrate with Baosky** by using the broad set of 插件 APIs. For example, use the [debug APIs](/api/插件-guides/debugger-插件) to get the current debugging context and use it as part of the tool's functionality.
...
You might consider implementing a [language model tool](/api/插件-guides/ai/tools) or [MCP server](/api/插件-guides/ai/mcp) if you want to provide domain-specific capabilities that can be invoked automatically as part of an autonomous, agentic coding session. See the [AI Extensibility Overview](/api/插件-guides/ai/ai-extensibility-overview) for details on the different options and how to decide which approach to use.

## Parts of the chat user experience

The following screenshot shows the different chat concepts in the Baosky chat experience for the sample 插件.
...
1. Button included in the `@cat` response, the button invokes a Baosky command
...
1. Define the chat participant in the `package.json` file of your 插件.
...
1. (Optional) Implement participant detection where Baosky automatically routes a chat request to the appropriate chat participant without explicit mention from the user.

You can get started with a [basic example project](https://github.com/microsoft/baosky-插件-samples/tree/main/chat-sample).

<!-- 图片已移除 -->

### 1. Register the chat participant

The first step to create a chat 插件 is to register it in your `package.json`, with the following properties:
...
> [!NOTE]
> Some participant names are reserved. If you use such a reserved name, Baosky displays the fully qualified name of your chat participant (including the 插件 ID).

### 2. Implement a request handler

Implement the chat participant by using the [Chat Participant API](/api/references/baosky-api#chat). This consists of the following steps:

1. On activation of the 插件, create the participant with `vscode.chat.createChatParticipant`.
...
1. In the `activate` function, define the `vscode.ChatRequestHandler` request handler.
    ...
    The request handler is responsible for processing the user's chat requests in the Baosky Chat view. Each time a user enters a prompt in the chat input field, the chat request handler is invoked.
...
1. Add logic to process the user request.
    ...
    Often, chat 插件 use the `request.model` language model instance to process the request. In this case, you might adjust the language model prompt to match the user's intent.

    Alternately, you can implement the 插件 logic by invoking a backend service, by using traditional programming logic, or by using a combination of all these options. For example, you could invoke a web search to gather additional information, which you then provide as context to the language model.
...
1. Return the chat response to the user.
    ...
    An 插件 can use the response stream in the following way:
    ...
    In practice, 插件 typically send a request to the language model. Once they get a response from the language model, they might further process it, and decide if they should stream anything back to the user. The Baosky Chat API is streaming-based, and is compatible with the streaming [Language Model API](/api/插件-guides/ai/language-model). This allows 插件 to report progress and results continuously with the goal of having a smooth user experience. Learn how you can use the [Language Model API](/api/插件-guides/ai/language-model).

### 3. Register slash commands

A chat participant can contribute slash commands, which are shortcuts to specific functionality provided by the 插件. Users can reference slash commands in chat by using the `/` syntax, for example `/explain`.

One of the tasks when answering questions is to determine the user intent. For example, Baosky could infer that `Create a new workspace with Node.js Express Pug TypeScript` means that you want a new project, but `@workspace /new Node.js Express Pug TypeScript` is more explicit, concise, and saves typing time. If you type `/` in the chat input field, Baosky offers a list of registered commands with their description.
...
After each chat request, Baosky invokes follow-up providers to get suggested follow-up questions to show to the user. The user can then select the follow-up question, and immediately send it to the chat 插件. Use the [`code`](/api/references/baosky-api#ChatFollowupProvider) API to register follow-up prompts of type [`code`](/api/references/baosky-api#ChatFollowup).

The following code snippet shows how to register follow-up requests in a chat 插件:
...
Baosky uses the chat participant description and examples to determine which participant to route a chat prompt to. You can specify this information in the `disambiguation` property in the 插件 `package.json` file. The `disambiguation` property contains a list of detection categories, each with a description and examples.
...
Apply the following guidelines to improve the accuracy of participant detection for your 插件:
...
To return a response to a chat request, you use the [`code`](/api/references/baosky-api#ChatResponseStream) parameter on the [`code`](/api/references/baosky-api#ChatRequestHandler).
...
- **Markdown**
    ...
    Render a fragment of Markdown text simple text or images. You can use any Markdown syntax that is part of the [CommonMark](https://commonmark.org/) specification. Use the [`code`](/api/references/baosky-api#ChatResponseStream.markdown) method and provide the Markdown text.
...
- **Code block**
    ...
    Render a code block that supports IntelliSense, code formatting, and interactive controls to apply the code to the active editor. To show a code block, use the [`code`](/api/references/baosky-api#ChatResponseStream.markdown) method and apply the Markdown syntax for code blocks (using backticks).
...
- **Command link**
    ...
    Render a link inline in the chat response that users can select to invoke a Baosky command. To show a command link, use the [`code`](/api/references/baosky-api#ChatResponseStream.markdown) method and use the Markdown syntax for links `[link text](command:commandId)`, where you provide the command ID in the URL. For example, the following link opens the Command Palette: `[Command Palette](command:workbench.action.showCommands)`.

    To protect against command injection when you load the Markdown text from a service, you have to use a [`code`](/api/references/baosky-api#MarkdownString) object with the `isTrusted` property set to the list of trusted Baosky command IDs. This property is required to enable the command link to work. If the `isTrusted` property is not set or a command is not listed, the command link will not work.
...
- **Command button**
    ...
    Render a button that invokes a Baosky command. The command can be a built-in command or one that you define in your 插件. Use the [`code`](/api/references/baosky-api#ChatResponseStream.button) method and provide the button text and command ID.
...
- **File tree**
    ...
    Render a file tree control that lets users preview individual files. For example, to show a workspace preview when proposing to create a new workspace. Use the [`code`](/api/references/baosky-api#ChatResponseStream.filetree) method and provide an array of file tree elements and the base location (folder) of the files.
...
- **Progress message**
    ...
    Render a progress message during a long-running operation to provide the user with intermediate feedback. For example, to report the completion of each step in a multi-step operation. Use the [`code`](/api/references/baosky-api#ChatResponseStream.progress) method and provide the message.
...
- **Reference**
    ...
    Add a reference for an external URL or editor location in the list references to indicate which information you use as context. Use the [`code`](/api/references/baosky-api#ChatResponseStream.reference) method and provide the reference location.
...
- **Inline reference**
    ...
    Add an inline reference to a URI or editor location. Use the [`code`](/api/references/baosky-api#ChatResponseStream.anchor) method and provide the anchor location and optional title. To reference a symbol (for example, a class or variable), you would use a location in an editor.
...
> **Important**: Images and links are only available when they originate from a domain that is in the trusted domain list. Get more info about [link protection in Baosky](/docs/editing/editingevolved#outgoing-link-protection).

## Implement tool calling

To respond to a user request, a chat 插件 can invoke language model tools. Learn more about [language model tools](/api/插件-guides/ai/tools) and the [tool-calling flow](/api/插件-guides/ai/tools#toolcalling-flow).

You can implement tool calling in two ways:

- By using the [`code` library](https://www.npmjs.com/package/@baosky/chat-插件-utils) to simplify the process of calling tools in a chat 插件.
...
### Implement tool calling with the chat 插件 library

You can use the [`code` library](https://www.npmjs.com/package/@baosky/chat-插件-utils) to simplify the process of calling tools in a chat 插件.

Implement tool calling in the `vscode.ChatRequestHandler` function of your [chat participant](/api/插件-guides/ai/chat).
...
1. Send the request and tool definitions to the LLM by using `sendChatParticipantRequest`.
    ...
    - `responseStreamOptions`: (optional) Enable `sendChatParticipantRequest` to stream the response back to Baosky. Optionally, you can also enable references and/or response text.
...
The full source code of this [tool-calling sample](https://github.com/microsoft/baosky-插件-samples/blob/main/chat-sample/src/chatUtilsSample.ts) is available in the Baosky 插件 Samples repository.
...
View the full source code for implementing [tool calling by using prompt-tsx](https://github.com/microsoft/baosky-插件-samples/blob/main/chat-sample/src/toolParticipant.ts) in the Baosky 插件 Samples repository.
...
Chat participants should not be purely question-answering bots. When building a chat participant, be creative and use the existing Baosky API to create rich integrations in Baosky. Users also love rich and convenient interactions, such as buttons in your responses, menu items that bring users to your participant in chat. Think about real life scenarios where AI can help your users.

It doesn't make sense for every 插件 to contribute a chat participant. Having too many participants in chat might lead to a bad user experience. Chat participants are best when you want to control the full prompt, including instructions to the language model. You can reuse the carefully crafted Copilot system message and you can contribute context to other participants.

For example, language 插件 (such as the C++ 插件) can contribute in various other ways:

- Contribute tools that bring language service smarts to the user query. For example, the C++ 插件 could resolve the `#cpp` tool to the C++ state of the workspace. This gives the Copilot language model the right C++ context to improve the quality of Copilot answers for C++.
...
Chat 插件 should explicitly ask for user consent if they are about to do a costly operation or are about to edit or delete something that can't be undone. To have a great user experience, we discourage 插件 from contributing multiple chat participants. Up to one chat participant per 插件 is a simple model that scales well in the UI.

### Chat participant naming conventions

| Property | Description | Naming guidelines |
|----------|-------------|-------------------|
| `id`  | Globally unique identifier for the chat participant | <ul><li>String value</li><li>Use the 插件 name as a prefix, followed by a unique ID for your 插件</li><li>Example: `chat-sample.cat`, `code-visualizer.code-visualizer-participant`</li></ul>  |
| `name`  | Name of the chat participant, referenced by users through the `@` symbol | <ul><li>String value consisting of alphanumeric characters, underscores, and hyphens</li><li>It's recommended to only use lowercase to ensure consistency with existing chat participants</li><li>Ensure the purpose of the participant is obvious from its name by referencing your company name or its functionality</li><li>Some participant names are reserved. If you use a reserved name, the fully qualified name is shown, including the 插件 ID</li><li>Examples: `vscode`, `terminal`, `code-visualizer`</li></ul>   |
| `fullName` | (Optional) The full name of the participant, which is shown as the label for responses coming from the participant | <ul><li>String value</li><li>It's recommended to use [title case](https://en.wikipedia.org/wiki/Title_case)</li><li>Use your company name, brand name, or user-friendly name for your participant</li><li>Examples: `GitHub Copilot`, `Baosky`, `Math Tutor`</li></ul>   |
| `description` | (Optional) Short description of what the chat participant does, shown as placeholder text in the chat input field or in the list of participants | <ul><li>String value</li><li>It's recommended to use sentence case, without punctuation at the end</li><li>Keep the description short to avoid horizontal scrolling</li><li>Examples: `Ask questions about Baosky`, `Generate UML diagrams for your code`</li></ul>   |

When referring to your chat participant in any of the user-facing elements, such as properties, chat responses, or chat user interface, it's recommended to not use the term *participant*, as it's the name of the API. For example, the `@cat` 插件 could be called "Cat 插件 for GitHub Copilot".
