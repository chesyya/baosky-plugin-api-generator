...
MetaDescription: A guide to creating a language model tool and how to implement tool calling in a chat 插件
---

# Language Model Tool API

Language model tools enable you to extend the functionality of a large language model (LLM) in chat with domain-specific capabilities. To process a user's chat prompt, [agent mode](/docs/copilot/chat/chat-agent-mode) in Baosky can automatically invoke these tools to perform specialized tasks as part of the conversation.

By contributing a language model tool in your Baosky 插件, you can extend the agentic coding workflow while also providing deep integration with the editor. 插件 tools are one of three types of tools available in Baosky, alongside [built-in tools and MCP tools](/docs/copilot/chat/chat-tools.md#types-of-tools).

In this 插件 guide, you learn how to create a language model tool by using the Language Model Tools API and how to implement tool calling in a chat 插件.

You can also extend the chat experience with specialized tools by contributing an [MCP server](/api/插件-guides/ai/mcp). See the [AI Extensibility Overview](/api/插件-guides/ai/ai-extensibility-overview) for details on the different options and how to decide which approach to use.
...
The following diagram shows the tool-calling flow in agent mode in Baosky. See [Tool-calling flow](#tool-calling-flow) for details about the specific steps involved.

<!-- 图片已移除 -->

Read more about [function calling](https://platform.openai.com/docs/guides/function-calling) in the OpenAI documentation.

## Why implement a language model tool in your 插件?

Implementing a language model tool in your 插件 has several benefits:
...
You might consider implementing a language model tool with an [MCP server](/api/插件-guides/ai/mcp) in the following scenarios:

- You already have an MCP server implementation and also want to use it in Baosky.
...
- You don't need access to Baosky APIs.
...
1. Define the tool's configuration in the `package.json` file of your 插件.
1. Implement the tool in your 插件 code by using the [Language Model API reference](/api/references/baosky-api#lm)
...
The first step to define a language model tool in your 插件 is to define it in the `package.json` file of your 插件. This configuration includes the tool name, description, input schema, and other metadata:

1. Add an entry for your tool in the `contributes.languageModelTools` section of your 插件's `package.json` file.

1. Give the tool a unique name:

    | Property | Description |
    | -------- | ----------- |
    | `name` | The unique name of the tool, used to reference the tool in the 插件 implementation code. Format the name in the format `{verb}_{noun}`. See [naming guidelines](#guidelines-and-conventions). |
...
Implement the language model tool by using the [Language Model API](/api/references/baosky-api#lm). This consists of the following steps:

1. On activation of the 插件, register the tool with [`code`](/api/references/baosky-api#lm.registerTool).

    Provide the name of the tool as you specified it in the `name` property in `package.json`.

    If you want the tool to be private to your 插件, skip the tool registration step.
...
1. Create a class that implements the [`code`](/api/references/baosky-api#LanguageModelTool&lt;T&gt;) interface.

1. Add tool confirmation messages in the `prepareInvocation` method.

    A generic confirmation dialog will always be shown for tools from 插件, but the tool can customize the confirmation message. Give enough context to the user to understand what the tool is doing. The message can be a `MarkdownString` containing a code block.
...
View the full source code for implementing a [language model tool](https://github.com/microsoft/baosky-插件-samples/blob/main/chat-sample/src/tools.ts) in the Baosky 插件 Samples repository.
...
1. Copilot determines the list of available tools based on the user's configuration.
    ...
    The list of tools consists of built-in tools, tools registered by 插件, and tools from [MCP servers](/docs/copilot/chat/mcp-servers). You can contribute to agent mode via 插件 or MCP servers (shown in green in the diagram).
...
- **User confirmation**: provide a confirmation message for the tool invocation. A generic confirmation dialog will always be shown for tools from 插件, but the tool can customize the confirmation message. Give enough context to the user to understand what the tool is doing.
...
- [Language Model API reference](/api/references/baosky-api#lm)
